...
#define B -1
/*パズルのボード上の空いている部分(以下では空白)をBと呼ぶことにする*/
...
void Board::shuffle(int tim)
{
  ...
  int N = rand() % 200 + 100;
  /*空白部分と数字を交換する操作を複数回行うことになるが、それを何回行うかを決める変数をNとした。平均的には26回程度で完全にシャッフルされるので、最低の交換回数は100回確保しておけば大丈夫かなという気持ちからNの値を上記のように設定した*/
  /*本当にこれで正しいのか不安*/
